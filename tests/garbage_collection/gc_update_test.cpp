...
class GCUpdateTest : public PelotonTest {};
...
TEST_F(GCUpdateTest, UpdateTest) {
  ...
  SeqScanCount(table, column_ids, predicate);
  ...
  after_update = catalog::Manager::GetInstance().GetMemoryFootprint();
  EXPECT_EQ(after_insert, after_update);
  tuple_id = 0;
  peloton::gc::GCManagerFactory::GetInstance().StopGC();
